...
export default {
  ...
  getIdByKeyword(keyword,tokenName){
      ...
      if(isNaN(keyword)){
        ...
        database.getRecordsByKeyword(keyword, tokenName,-1, constants.WCS_STORE_ID).then(function(response) {
          logger.info("Response" + JSON.stringify(response));
          if (Object.keys(response[0]).length > 0) {
            var keyword = response[0].URLKEYWORD;
            var tokenType = response[0].TOKENNAME;
        ...
        database.getRecordsByProductId(keyword, tokenName,-1, constants.WCS_STORE_ID).then(function(response) {
          logger.info("Response" + JSON.stringify(response));
          if (Object.keys(response[0]).length > 0) {
            var keyword = response[0].URLKEYWORD;
            var tokenType = response[0].TOKENNAME;
