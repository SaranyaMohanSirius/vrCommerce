...
module.exports = {
  ...
  selectShippingAddress: function(req,res){
    ...
    getBillingAddresses: function(req,res){
        ...
        let getBillingAddressURL = constructUrl(constants.EP_HOSTNAME_CORTEX, conCatUrl, false);   
        logger.info('getBillingAddress url: ',  getBillingAddressURL);
        let method ='GET';
        let requestCall = constructRequest(getBillingAddressURL,method,messageData,token)

        requestPromise(requestCall).then(function (data) {
            logger.info("data",data);
           let result = addressMapper.getBillingAddressesJSON(data); 
                res.send({
                  "success": true ,
                  "result": result,                                            
              });   
  ...
  selectBillingAddress: function(req,res){
        ...
        let conCatUrl = req.body.orderId + constants.EP_GET_BILLING_ADDRESS_SELECTOR + req.body.addressId + constants.EP_FOLLOW_LOCATION;;

        let selectBillingAddressURL = constructUrl(constants.EP_HOSTNAME_CORTEX,conCatUrl,false);

        logger.info('selectBillingAddressURL url: ',  selectBillingAddressURL);
        let method ='POST';
        let requestCall = constructRequest(selectBillingAddressURL,method,messageData,token)

        requestPromise(requestCall).then(function (data) {
         let result = addressMapper.selectBillingAddressJSON(data); 
              res.send({
                "success": true ,
                "result": result,                                            
            });   
